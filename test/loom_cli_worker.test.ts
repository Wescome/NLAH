...
describe("LoomCliWorkerAdapter", () => {
  ...
  describe("error handling", () => {
    ...
    it("redacts api keys in Loom failure diagnostics", async () => {
      const { artifacts, input, root } = await fixture();
      const shell = new FakeShell([
        {
          ok: false,
      ...
      const worker = new LoomCliWorkerAdapter({ extraArgs: ["--api-key", "secret-loom-key"] }, shell);
      ...
      expect(message).not.toContain("secret-loom-key");

      const command = JSON.parse(await readFile(path.join(root, "debug", "loom.command.json"), "utf8")) as {
        command: string[];
      };
      expect(command.command).toContain("[redacted]");
      expect(command.command).not.toContain("secret-loom-key");
    });
